...
import('./js/data/constants.js').then(module => {
    typeChart = module.typeChart;
    console.log('[INIT] Loaded complete Type Chart for secure Server Calculations');
}).catch(e => console.error('[INIT] Failed to load typeChart:', e));

function calculateDamage(offStat, defStat, defenderTypes, movePower, moveType) {
    const effectiveness = defenderTypes.reduce((multiplier, defType) => {
        const chart = typeChart[moveType];
        return (chart && chart[defType] !== undefined) ? multiplier * chart[defType] : multiplier;
    }, 1);
    if (effectiveness === 0) return { damage: 0, effectiveness: 0 };
    const statDifference = offStat - defStat;
    const damage = statDifference > 0 ? Math.max(1, Math.floor(movePower * effectiveness * statDifference / 100)) : 0;
...
io.on('connection', (socket) => {
    ...
    socket.on('game:calculate_damage', (data, callback) => {
        try {
            const { offStat, defStat, defenderTypes, movePower, moveType } = data;
            const result = calculateDamage(offStat, defStat, defenderTypes, movePower, moveType);
            callback({ success: true, ...result });
        } catch(error) {
            console.error('[ERROR] game:calculate_damage', error);
            callback({ success: false, error: error.message });
        }
    });
