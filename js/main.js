...
export class PokemonBattleArena {
    ...
    _openPokemonEditor(slotIndex) {
        ...
        if (typeof window.jQuery !== 'undefined') {
            $('#edit-pokemon-select').select2({ theme: 'classic', width: '100%', dropdownParent: $('#pokemon-editor-form') });
            $('#edit-pokemon-select').on('select2:select', function (e) {
                abilitySelect.innerHTML = window.arena._buildAbilityOptionsHTML(e.params.data.id);
            });
            if (existingMon) $('#edit-pokemon-select').val(existingMon.name).trigger('change');
        }
    ...
    _buildAbilityOptionsHTML(pokemonName, selectedAbility = null) {
        ...
        const data = this.db.getPokemon(pokemonName);
    ...
    _confirmPokemonEdit() {
        ...
        if (typeof window.jQuery !== 'undefined') pokemonName = $('#edit-pokemon-select').val();
    ...
    _populateMoveSelector() {
        if (typeof window.jQuery === 'undefined' || !window.movesData) return;
        const moveSelect = $('#move-name-select');
        moveSelect.empty().append('<option value="">-- Select Move --</option>');
        ...
        sortedMoves.forEach(m => {
            ...
            moveSelect.append(`<option value="${m.name}" data-type="${m.type}" data-power="${m.basePower}">${m.name} (${m.type}, ${bp})</option>`);
        });
        moveSelect.select2({ theme: 'classic', width: '100%', placeholder: '-- Select Move --', allowClear: true });
        
        moveSelect.on('select2:select', function(e) {
            const m = e.params.data;
            const $opt = $(m.element);
            const type = $opt.data('type');
            const power = $opt.data('power');
            let category = null;
            if (window.movesData) {
                const moveData = Object.values(window.movesData).find(x => x.name === m.id);
                if (moveData) category = moveData.category; // "Physical", "Special", or "Status"
            }
            if (type) $('#move-type-select').val(type).trigger('change');
            if (power && power !== '--') $('#move-power-input').val(power);
            else $('#move-power-input').val('');
            
            if (category) {
                if (category === 'Physical') { window.arena.handleAttack('Physical'); }
                else if (category === 'Special') { window.arena.handleAttack('Special'); }
                else { window.arena._announce(`${m.id} is a Status move. Implementing effects is manual.`, false); }
            }
        });
    }
...
window.shareRoomLink = function() {
    ...
    if (navigator.share) {
        navigator.share({ title: 'Pokémon Battle Arena - Join My Room!', text: shareText, url: joinUrl }).catch(() => copyToClipboardWithFeedback(joinUrl));
    } else {
